...
export default function ReportsPage() {
  ...
  const handleTriggerEmail = async () => {
    ...
    try {
      ...
      alert(res || "Email task completed!");
    } catch (e) {
      alert("Failed to trigger email: " + (e.message || ""));
    } finally {
      setEmailing(false);
    }
  };
  ...
  const categoryMap = {};
  products.forEach(p => {
    const catName = p.category || 'Unknown';
        ...
        <div className="page-inner fade-up">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 28 }}>
            <div>
              ...
              <p className="page-subtitle" style={{ marginBottom: 0 }}>Inventory analytics and detailed breakdowns.</p>
            </div>
            {hasRole('SUPPLIER') && (
              <button 
              ...
              >
                {emailing ? 'Sending…' : '✉ Send Low Stock Email'}
              </button>
            )}
          </div>
